...
import static edu.byu.cs.tweeter.net.FollowGenerator.Sort.FOLLOWEE_FOLLOWER;
import static edu.byu.cs.tweeter.net.FollowGenerator.Sort.FOLLOWER_FOLLOWEE;
...
public class FollowGenerator {
    ...
    public List<Follow> generateUsersAndFollows(int userCount, int minFollowersPerUser,
                                                       int maxFollowersPerUser, Sort sortOrder, User user) {
        List<User> users = UserGenerator.getInstance().generateUsers(userCount);
        return generateFollowsForUsers(users, minFollowersPerUser, maxFollowersPerUser, sortOrder, user);
    }
    ...
    public List<Follow> generateFollowsForUsers(@NotNull List<User> users,
        ...
        for(User user : users) {
            ...
            generateFollowersForUser(numbFollowersToGenerate, user, users, allFollows);
        }

        // Add the test user and make him follow everyone
        User testUser = loginUser;
        ...
        sortFollowArray(allFollows, sortOrder);
    ...
    public static List<Follow> sortFollowArray(List<Follow> follows, Sort sortOrder) {
        switch (sortOrder) {
