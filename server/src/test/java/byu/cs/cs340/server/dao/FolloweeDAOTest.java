...
class FolloweeDAOTest {
    ...
    private FollowerDAO followeeDAOSpy;

    @BeforeEach
    void setup() {
        followeeDAOSpy = Mockito.spy(new FollowerDAO());

        FollowGenerator mockFollowGenerator = Mockito.mock(FollowGenerator.class);
        Mockito.when(mockFollowGenerator.generateUsersAndFollows(Mockito.anyInt(), Mockito.anyInt(), (FollowGenerator.Sort) Mockito.any())).thenReturn(follows);

        Mockito.when(followeeDAOSpy.getFollowGenerator()).thenReturn(mockFollowGenerator);
    }

    @Test
    void testGetFollowees_noFolloweesForUser() {

        FolloweeRequest request = new FolloweeRequest(user1, 10, null);
        FolloweeResponse response = followeeDAOSpy.getFollowees(request);
    ...
    void testGetFollowees_oneFollowerForUser_limitGreaterThanUsers() {
        ...
        FolloweeRequest request = new FolloweeRequest(user9, 10, null);
        FolloweeResponse response = followeeDAOSpy.getFollowees(request);
    ...
    void testGetFollowees_twoFollowersForUser_limitEqualsUsers() {
        ...
        FolloweeRequest request = new FolloweeRequest(user3, 2, null);
        FolloweeResponse response = followeeDAOSpy.getFollowees(request);

        Assertions.assertEquals(2, response.getFollowers().size());
        Assertions.assertTrue(response.getFollowers().contains(user1));
    ...
    void testGetFollowees_limitLessThanUsers_endsOnPageBoundary() {
        ...
        FolloweeRequest request = new FolloweeRequest(user5, 2, null);
        FolloweeResponse response = followeeDAOSpy.getFollowees(request);

        // Verify first page
        Assertions.assertEquals(2, response.getFollowers().size());
        Assertions.assertTrue(response.getFollowers().contains(user9));
        Assertions.assertTrue(response.getFollowers().contains(user11));
        Assertions.assertTrue(response.getHasMorePages());

        // Get and verify second page
        request = new FolloweeRequest(user5, 2, response.getFollowers().get(1));
        response = followeeDAOSpy.getFollowees(request);
    ...
    void testGetFollowees_limitLessThanUsers_notEndsOnPageBoundary() {
        ...
        FolloweeRequest request = new FolloweeRequest(user6, 2, null);
        FolloweeResponse response = followeeDAOSpy.getFollowees(request);
        ...
        // Get and verify second page
        request = new FolloweeRequest(user6, 2, response.getFollowers().get(1));
        response = followeeDAOSpy.getFollowees(request);
        ...
        Assertions.assertEquals(2, response.getFollowers().size());
        Assertions.assertTrue(response.getFollowers().contains(user10));
        Assertions.assertTrue(response.getFollowers().contains(user12));
        Assertions.assertTrue(response.getHasMorePages());
